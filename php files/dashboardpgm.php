<?php include("connection1.php"); $pd = ""; session_start(); ?>
<html>
    <head>
        <title>Dashboard/PGM</title>
        <script>
    ...
    <body>
        ...
        <table>
            <tr>
                ...
                <td width = "*" align = "right"><img src = "images/question.png"><a href="home.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Help</a> | <img src = "images/contact.png">&thinsp;<a href="contact.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Contact</a> | <img src = "images/Logout.png">&thinsp;<a href = "logout.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Logout</a></td>
            </tr>
        </table>
        <hr><br><br><br>
        <fieldset>
                        ...
                        <table>
                            <tr>
                                ...
                                <td><select class = "sel" id = "sel" name = "sel[]" required>
                                        ...
                                        <?php
                                            $que = mysql_query("select ProjectManager from projectdetails where ProgramManager = '".$_SESSION['user']."' group by ProjectManager;");
                                            if(mysql_num_rows($que))
                                            {
                                                while($rs = mysql_fetch_array($que))
                                                {
                                                    echo "<option value=".$rs['ProjectManager']." >".$rs['ProjectManager']."</option>";  
                                                }
                                            }
                                        ?>
                                    </select></td>
...
<!----------------------------------------------------------------------------------------------------------------------------------------->                        
        ...
        <fieldset><center>
                    <h2 style="text-align:center;" id = "title">Projects</h2>
                    <?php
                            ...
                            if(mysql_num_rows($tab))
                            {
                                while($tabv = mysql_fetch_assoc($tab))
                                {
                                    ...
                                    <tr>
                                        ...
                                        <?php echo $tabv['Name']; ?></td>
                                        <td style="border: 1px solid #DDD; text-align: left; padding: 15px;">
                                        <?php echo $tabv['ProjectManager']; ?></td>
    ...
    //					alert(response);   // check the response which we get in response
                        ...
                        $('.pj').append(response); // append the html content to the "select" tag whichever you want... i just appended it to "pgm" select dropdown
...
<?php
    if(isset($_POST["submit"]))
    {
        ...
        {
            $_SESSION['pmsel'] = $temp1;
        }
        foreach($_REQUEST['pj'] as $temp2)
        {
            $_SESSION['pn'] = $temp2;
        }
        ...
        ?><script>//alert("<?php //echo $_SESSION['pmsel'].$_SESSION['pn'].$_SESSION['pid'].$_SESSION['user']; ?>");//location.href = "dashboardpgm.php";</script><?php
        
        $checkque = mysql_query("SELECT Name FROM projectdetails WHERE ProjectManager = '".$_SESSION['pmsel']."' AND ProgramManager = '".$_SESSION['user']."' AND ProjectID = '".$_SESSION['pid']."';");
        
        $see = mysql_fetch_assoc($checkque);
        
        if($_SESSION['pn'] != $see['Name'])
        {
            ?><script>alert("Sorry, the selected option with your profile doesn't match with any project."); alert("Please select proper options.");location.href = "dashboardpgm.php";</script><?php
        }
        if($_SESSION['pn'] == $see['Name'])
        {
