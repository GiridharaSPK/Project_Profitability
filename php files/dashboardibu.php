    ...
    <body>
        ...
        <table>
            <tr>
                ...
                <td width = "*" align = "right"><img src = "images/question.png"><a href="images/home.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Help</a> | <img src = "images/contact.png">&thinsp;<a href="contact.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Contact</a> | <img src = "images/Logout.png">&thinsp;<a href = "logout.php" style = "color:#000000; text-decoration:none" onMouseOver = "this.style.color = '#FF0000'" onMouseOut = "this.style.color = '#000000'">Logout</a></td>
            </tr>
        </table>
        <hr><br><br><br>
        <fieldset>
                        ...
                        <table>
                            <tr>
                                ...
                                <td><select class = "sel" id = "sel" name = "sel[]" required>
                                        ...
                                        <?php
                                            $que = mysql_query("select ProgramManager from projectdetails  where IBUHead = '".$_SESSION['user']."' group by ProgramManager;");
                                            if(mysql_num_rows($que))
                                            {
                                                while($rs = mysql_fetch_array($que))
                                                {
                                                    echo "<option value=".$rs['ProgramManager']." >".$rs['ProgramManager']."</option>";  
                                                }
                                            }
                                        ?>
                                    </select></td>
...
<!--------------------------------------------------------------------------------------------------------------------------------------------------->                        
                            <tr style="background-color:#FFFFFF">
                                ...
                                <td><select class= "pm" id = "pm" name = "pm[]" required>
                                    <option value = "">----Project Manager----</option>                                 <div class="temp"></div>                            
                                    </select></td>
                            </tr>
<!--------------------------------------------------------------------------------------------------------------------------------------------------->                    
                            <tr>
...
//					alert(response);   // check the response which we get in response
					...
					$('.pm').append(response); // append the html content to the "select" tag whichever you want... i just appended it to "pgm" select dropdown
...
<?php
    if(isset($_POST["submit"]))
    {
        ...
        ?><script>//alert("<?php //echo $_SESSION['pgmsel'].$_SESSION['pmsel'].$_SESSION['pn'].$_SESSION['pid'].$_SESSION['user']; ?>");//location.href = "dashboardibu.php";</script><?php

        $checkque = mysql_query("SELECT Name FROM projectdetails WHERE ProjectManager = '".$_SESSION['pmsel']."' AND ProgramManager = '".$_SESSION['pgmsel']."' AND IBUHead = '".$_SESSION['user']."' AND ProjectID = '".$_SESSION['pid']."';");

        $see = mysql_fetch_assoc($checkque);
        if($_SESSION['pn'] != $see['Name'])
        {
            ?><script>alert("Sorry, the selected option with your profile doesn't match with any project."); alert("Please select proper options.");location.href = "dashboardibu.php";</script><?php
        }
        if($_SESSION['pn'] == $see['Name'])
        {
